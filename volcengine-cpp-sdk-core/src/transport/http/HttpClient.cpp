...
namespace volcengine {
    ...
    static size_t receiveBody(char *ptr, size_t size, size_t nmemb, void *userdata) {
        ...
        if (resourceMan->firstReceive) {
            ...
            resourceMan->firstReceive = false;
        }
    ...
    static size_t recvHeaders(char *buffer, size_t size, size_t nitems, void *userdata) {
        ...
        if (length == 2 && (buffer[0] == 0x0D) && (buffer[1] == 0x0A)) {
            if (resourceMan->httpResp->hasHeader(HEADER_CONTENT_LENGTH)) {
                double dval;
...
//std::cout << url << std::endl;
    ...
    // add user-agent
    curl_easy_setopt(curl, CURLOPT_USERAGENT, volcengine::DefaultUserAgent().c_str());
