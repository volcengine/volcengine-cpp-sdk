...
void DescribeDirectConnectConnectionsResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("DirectConnectConnections")){
        json directConnectConnectionsPres = j.at("DirectConnectConnections");
        for (auto & pre : directConnectConnectionsPres) {
           DirectConnectConnectionForDescribeDirectConnectConnectionsOutput next;
           next.fromJsonString(to_string(pre));
           v_directConnectConnections.emplace_back(next);
        }
    }
