...
void DescribeDirectConnectVirtualInterfacesResult::fromJsonString(const std::string &input) {
    ...
    if (j.contains("VirtualInterfaces")){
        json virtualInterfacesPres = j.at("VirtualInterfaces");
        for (auto & pre : virtualInterfacesPres) {
           VirtualInterfaceForDescribeDirectConnectVirtualInterfacesOutput next;
           next.fromJsonString(to_string(pre));
           v_virtualInterfaces.emplace_back(next);
        }
    }
}
