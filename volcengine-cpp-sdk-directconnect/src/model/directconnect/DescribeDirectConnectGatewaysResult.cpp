...
void DescribeDirectConnectGatewaysResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("DirectConnectGateways")){
        json directConnectGatewaysPres = j.at("DirectConnectGateways");
        for (auto & pre : directConnectGatewaysPres) {
           DirectConnectGatewayForDescribeDirectConnectGatewaysOutput next;
           next.fromJsonString(to_string(pre));
           v_directConnectGateways.emplace_back(next);
        }
    }
