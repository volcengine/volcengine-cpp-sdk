...
void DescribeDirectConnectGatewayRoutesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("DirectConnectGatewayRoutes")){
        json directConnectGatewayRoutesPres = j.at("DirectConnectGatewayRoutes");
        for (auto & pre : directConnectGatewayRoutesPres) {
           DirectConnectGatewayRouteForDescribeDirectConnectGatewayRoutesOutput next;
           next.fromJsonString(to_string(pre));
           v_directConnectGatewayRoutes.emplace_back(next);
        }
    }
