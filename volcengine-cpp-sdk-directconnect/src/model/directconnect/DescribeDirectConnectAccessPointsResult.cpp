...
void DescribeDirectConnectAccessPointsResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("DirectConnectAccessPoints")){
        json directConnectAccessPointsPres = j.at("DirectConnectAccessPoints");
        for (auto & pre : directConnectAccessPointsPres) {
           DirectConnectAccessPointForDescribeDirectConnectAccessPointsOutput next;
           next.fromJsonString(to_string(pre));
           v_directConnectAccessPoints.emplace_back(next);
        }
    }
