...
void DescribeBgpPeersResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("BgpPeers")){
        json bgpPeersPres = j.at("BgpPeers");
        for (auto & pre : bgpPeersPres) {
           BgpPeerForDescribeBgpPeersOutput next;
           next.fromJsonString(to_string(pre));
           v_bgpPeers.emplace_back(next);
        }
    }
