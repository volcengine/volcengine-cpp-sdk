...
void DescribeRulesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("RequestId")) j.at("RequestId").get_to(v_requestId);
    if (j.contains("Rules")){
        json rulesPres = j.at("Rules");
        for (auto & pre : rulesPres) {
           RuleForDescribeRulesOutput next;
           next.fromJsonString(to_string(pre));
           v_rules.emplace_back(next);
        }
    }
}
