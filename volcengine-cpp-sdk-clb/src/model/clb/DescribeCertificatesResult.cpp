...
void DescribeCertificatesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("Certificates")){
        json certificatesPres = j.at("Certificates");
        for (auto & pre : certificatesPres) {
           CertificateForDescribeCertificatesOutput next;
           next.fromJsonString(to_string(pre));
           v_certificates.emplace_back(next);
        }
    }
