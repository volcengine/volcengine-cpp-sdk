...
void ResultForDescribeListenerHealthOutput::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    if (j.contains("InstanceId")) j.at("InstanceId").get_to(v_instanceId);
    if (j.contains("Ip")) j.at("Ip").get_to(v_ip);
    if (j.contains("Port")) j.at("Port").get_to(v_port);
    if (j.contains("RuleNumber")) j.at("RuleNumber").get_to(v_ruleNumber);
    if (j.contains("ServerGroupId")) j.at("ServerGroupId").get_to(v_serverGroupId);
    if (j.contains("ServerId")) j.at("ServerId").get_to(v_serverId);
