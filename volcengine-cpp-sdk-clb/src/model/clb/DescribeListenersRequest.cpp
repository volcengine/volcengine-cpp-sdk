...
RequestBuilder DescribeListenersRequest::withRequestBuilder(RequestBuilder rb, const std::string &chain) const {
    rb.withQuery("Action","DescribeListeners");
    rb.withQuery("Version","2020-04-01");

    if(v_listenerIdsHasBeenSet){
      unsigned v_listenerIdsCount = 1;
      for(auto& item : v_listenerIds){
        std::stringstream ss;
        ss << chain << "ListenerIds." << v_listenerIdsCount;
