...
void DescribeLoadBalancersResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("LoadBalancers")){
        json loadBalancersPres = j.at("LoadBalancers");
        for (auto & pre : loadBalancersPres) {
           LoadBalancerForDescribeLoadBalancersOutput next;
           next.fromJsonString(to_string(pre));
           v_loadBalancers.emplace_back(next);
        }
    }
