...
RequestBuilder DescribeCertificatesRequest::withRequestBuilder(RequestBuilder rb, const std::string &chain) const {
    rb.withQuery("Action","DescribeCertificates");
    rb.withQuery("Version","2020-04-01");

    if(v_certificateIdsHasBeenSet){
      unsigned v_certificateIdsCount = 1;
      for(auto& item : v_certificateIds){
        std::stringstream ss;
        ss << chain << "CertificateIds." << v_certificateIdsCount;
        rb.withQuery(ss.str(), item);
        v_certificateIdsCount ++;
      }
    }

    if(v_certificateNameHasBeenSet){
        rb.withQuery(chain + "CertificateName", v_certificateName);
