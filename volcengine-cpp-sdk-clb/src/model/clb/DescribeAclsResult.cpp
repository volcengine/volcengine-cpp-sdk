...
void DescribeAclsResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("Acls")){
        json aclsPres = j.at("Acls");
        for (auto & pre : aclsPres) {
           AclForDescribeAclsOutput next;
           next.fromJsonString(to_string(pre));
           v_acls.emplace_back(next);
        }
    }
