...
void DescribeLoadBalancersBillingResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("LoadBalancerBillingConfigs")){
        json loadBalancerBillingConfigsPres = j.at("LoadBalancerBillingConfigs");
        for (auto & pre : loadBalancerBillingConfigsPres) {
           LoadBalancerBillingConfigForDescribeLoadBalancersBillingOutput next;
           next.fromJsonString(to_string(pre));
           v_loadBalancerBillingConfigs.emplace_back(next);
        }
    }
