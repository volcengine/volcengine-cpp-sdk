...
void DescribeNetworkInterfacesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("NetworkInterfaceSets")){
        json networkInterfaceSetsPres = j.at("NetworkInterfaceSets");
        for (auto & pre : networkInterfaceSetsPres) {
           NetworkInterfaceSetForDescribeNetworkInterfacesOutput next;
           next.fromJsonString(to_string(pre));
           v_networkInterfaceSets.emplace_back(next);
        }
    }
