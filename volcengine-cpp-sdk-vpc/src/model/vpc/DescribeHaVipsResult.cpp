...
void DescribeHaVipsResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("HaVips")){
        json haVipsPres = j.at("HaVips");
        for (auto & pre : haVipsPres) {
           HaVipForDescribeHaVipsOutput next;
           next.fromJsonString(to_string(pre));
           v_haVips.emplace_back(next);
        }
    }
