...
void DescribeSecurityGroupsResult::fromJsonString(const std::string &input) {
    ...
    if (j.contains("SecurityGroups")){
        json securityGroupsPres = j.at("SecurityGroups");
        for (auto & pre : securityGroupsPres) {
           SecurityGroupForDescribeSecurityGroupsOutput next;
           next.fromJsonString(to_string(pre));
           v_securityGroups.emplace_back(next);
        }
    }
