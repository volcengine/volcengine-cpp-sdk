...
void DescribeRouteEntryListResult::fromJsonString(const std::string &input) {
    ...
    if (j.contains("RouteEntries")){
        json routeEntriesPres = j.at("RouteEntries");
        for (auto & pre : routeEntriesPres) {
           RouteEntryForDescribeRouteEntryListOutput next;
           next.fromJsonString(to_string(pre));
           v_routeEntries.emplace_back(next);
        }
    }
