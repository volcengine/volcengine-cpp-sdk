...
void DescribeNetworkAclsResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("NetworkAcls")){
        json networkAclsPres = j.at("NetworkAcls");
        for (auto & pre : networkAclsPres) {
           NetworkAclForDescribeNetworkAclsOutput next;
           next.fromJsonString(to_string(pre));
           v_networkAcls.emplace_back(next);
        }
    }
