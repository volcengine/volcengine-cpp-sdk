...
void DescribeVpcsResult::fromJsonString(const std::string &input) {
    ...
    if (j.contains("Vpcs")){
        json vpcsPres = j.at("Vpcs");
        for (auto & pre : vpcsPres) {
           VpcForDescribeVpcsOutput next;
           next.fromJsonString(to_string(pre));
           v_vpcs.emplace_back(next);
        }
    }
}
