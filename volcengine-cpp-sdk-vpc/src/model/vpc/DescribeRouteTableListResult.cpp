...
void DescribeRouteTableListResult::fromJsonString(const std::string &input) {
    ...
    if (j.contains("RouterTableList")){
        json routerTableListPres = j.at("RouterTableList");
        for (auto & pre : routerTableListPres) {
           RouterTableListForDescribeRouteTableListOutput next;
           next.fromJsonString(to_string(pre));
           v_routerTableList.emplace_back(next);
        }
    }
