...
void DescribeBandwidthPackagesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("BandwidthPackages")){
        json bandwidthPackagesPres = j.at("BandwidthPackages");
        for (auto & pre : bandwidthPackagesPres) {
           BandwidthPackageForDescribeBandwidthPackagesOutput next;
           next.fromJsonString(to_string(pre));
           v_bandwidthPackages.emplace_back(next);
        }
    }
