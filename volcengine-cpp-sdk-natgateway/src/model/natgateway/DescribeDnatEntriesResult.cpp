...
void DescribeDnatEntriesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("DnatEntries")){
        json dnatEntriesPres = j.at("DnatEntries");
        for (auto & pre : dnatEntriesPres) {
           DnatEntryForDescribeDnatEntriesOutput next;
           next.fromJsonString(to_string(pre));
           v_dnatEntries.emplace_back(next);
        }
    }
