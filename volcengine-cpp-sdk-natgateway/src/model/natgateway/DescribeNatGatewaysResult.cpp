...
void DescribeNatGatewaysResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("NatGateways")){
        json natGatewaysPres = j.at("NatGateways");
        for (auto & pre : natGatewaysPres) {
           NatGatewayForDescribeNatGatewaysOutput next;
           next.fromJsonString(to_string(pre));
           v_natGateways.emplace_back(next);
        }
    }
