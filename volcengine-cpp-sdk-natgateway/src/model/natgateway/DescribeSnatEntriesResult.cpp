...
void DescribeSnatEntriesResult::fromJsonString(const std::string &input) {
    ...
    if (j.contains("SnatEntries")){
        json snatEntriesPres = j.at("SnatEntries");
        for (auto & pre : snatEntriesPres) {
           SnatEntryForDescribeSnatEntriesOutput next;
           next.fromJsonString(to_string(pre));
           v_snatEntries.emplace_back(next);
        }
    }
