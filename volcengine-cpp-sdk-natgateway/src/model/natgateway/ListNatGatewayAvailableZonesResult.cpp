...
void ListNatGatewayAvailableZonesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("RequestId")) j.at("RequestId").get_to(v_requestId);
    if (j.contains("Zones")){
        json zonesPres = j.at("Zones");
        for (auto & pre : zonesPres) {
           ZoneForListNatGatewayAvailableZonesOutput next;
           next.fromJsonString(to_string(pre));
           v_zones.emplace_back(next);
        }
    }
}
