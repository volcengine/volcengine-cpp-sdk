...
void DescribeCensResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("Cens")){
        json censPres = j.at("Cens");
        for (auto & pre : censPres) {
           CenForDescribeCensOutput next;
           next.fromJsonString(to_string(pre));
           v_cens.emplace_back(next);
        }
    }
