...
void DescribeCenAttachedInstancesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("AttachedInstances")){
        json attachedInstancesPres = j.at("AttachedInstances");
        for (auto & pre : attachedInstancesPres) {
           AttachedInstanceForDescribeCenAttachedInstancesOutput next;
           next.fromJsonString(to_string(pre));
           v_attachedInstances.emplace_back(next);
        }
    }
