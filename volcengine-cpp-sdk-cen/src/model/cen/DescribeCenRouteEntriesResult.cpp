...
void DescribeCenRouteEntriesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("CenRouteEntries")){
        json cenRouteEntriesPres = j.at("CenRouteEntries");
        for (auto & pre : cenRouteEntriesPres) {
           CenRouteEntryForDescribeCenRouteEntriesOutput next;
           next.fromJsonString(to_string(pre));
           v_cenRouteEntries.emplace_back(next);
        }
    }
