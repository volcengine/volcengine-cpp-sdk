...
void DescribeInstanceGrantedRulesResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("CenGrantRules")){
        json cenGrantRulesPres = j.at("CenGrantRules");
        for (auto & pre : cenGrantRulesPres) {
           CenGrantRuleForDescribeInstanceGrantedRulesOutput next;
           next.fromJsonString(to_string(pre));
           v_cenGrantRules.emplace_back(next);
        }
    }
