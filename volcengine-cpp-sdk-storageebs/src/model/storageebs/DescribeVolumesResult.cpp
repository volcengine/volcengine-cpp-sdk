...
void DescribeVolumesResult::fromJsonString(const std::string &input) {
    ...
    if (j.contains("Volumes")){
        json volumesPres = j.at("Volumes");
        for (auto & pre : volumesPres) {
           VolumeForDescribeVolumesOutput next;
           next.fromJsonString(to_string(pre));
           v_volumes.emplace_back(next);
        }
    }
}
