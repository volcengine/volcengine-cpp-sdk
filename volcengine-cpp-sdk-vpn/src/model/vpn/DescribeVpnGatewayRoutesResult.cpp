...
void DescribeVpnGatewayRoutesResult::fromJsonString(const std::string &input) {
    ...
    if (j.contains("VpnGatewayRoutes")){
        json vpnGatewayRoutesPres = j.at("VpnGatewayRoutes");
        for (auto & pre : vpnGatewayRoutesPres) {
           VpnGatewayRouteForDescribeVpnGatewayRoutesOutput next;
           next.fromJsonString(to_string(pre));
           v_vpnGatewayRoutes.emplace_back(next);
        }
    }
}
