...
void DescribeVpnConnectionsResult::fromJsonString(const std::string &input) {
    ...
    if (j.contains("VpnConnections")){
        json vpnConnectionsPres = j.at("VpnConnections");
        for (auto & pre : vpnConnectionsPres) {
           VpnConnectionForDescribeVpnConnectionsOutput next;
           next.fromJsonString(to_string(pre));
           v_vpnConnections.emplace_back(next);
        }
    }
}
