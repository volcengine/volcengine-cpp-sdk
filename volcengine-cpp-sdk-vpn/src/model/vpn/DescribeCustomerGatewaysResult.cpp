...
void DescribeCustomerGatewaysResult::fromJsonString(const std::string &input) {
    auto j = json::parse(input);
    j = j.at("Result");
    if (j.contains("CustomerGateways")){
        json customerGatewaysPres = j.at("CustomerGateways");
        for (auto & pre : customerGatewaysPres) {
           CustomerGatewayForDescribeCustomerGatewaysOutput next;
           next.fromJsonString(to_string(pre));
           v_customerGateways.emplace_back(next);
        }
    }
